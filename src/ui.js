...
document.addEventListener('keydown', e => {
  ...
  if (e.key === 'Escape') {
    // Native fullscreen gets Escape from the browser; the fallback has to
    // provide it, or the only way out is the button.
    if (inPseudoFullscreen()) { e.preventDefault(); pseudoFullscreen(false); return; }
...
cnv.addEventListener('pointerdown', e => {
  ...
  if (activePointers.size >= 2) return; // a pinch owns this gesture, not a drag or a wire
...
cnv.addEventListener('touchmove', e => { if (drag || activePointers.size >= 2) e.preventDefault(); }, { passive: false });
...
// ---- pinch to zoom ----
// #cnv and its whole subtree carry touch-action:none, which is what stops iOS
// handing a mid-drag gesture to the page (issue #15). The side effect is that
// the browser's own pinch never reaches the canvas either, and there was no
// handler of our own, so pinching a phone did nothing at all. Note the default
// would have been wrong anyway: the browser pinch scales the whole PAGE, and
// what someone pinching a schematic wants is the camera.
//
// Two fingers zoom AND pan together, the standard map gesture: the world point
// under the midpoint of the two fingers stays put, so you can zoom into a
// corner without a separate drag. Reuses zoomAt() so the anchoring maths is the
// same one the mouse wheel uses.
const activePointers = new Map();
let pinch = null; // { dist, mid } from the previous move
function pinchGeometry() {
  const p = [...activePointers.values()];
  const dx = p[0].x - p[1].x, dy = p[0].y - p[1].y;
  return { dist: Math.hypot(dx, dy), mid: { x: (p[0].x + p[1].x) / 2, y: (p[0].y + p[1].y) / 2 } };
}
cnv.addEventListener('pointerdown', e => {
  if (e.pointerType === 'mouse') return;
  activePointers.set(e.pointerId, { x: e.clientX, y: e.clientY });
  if (activePointers.size === 2) {
    // A pinch is starting, so whatever the first finger began is not what the
    // user meant. Abandon it rather than dragging a block across the canvas
    // while they zoom.
    drag = null;
    S.wireFrom = null; wirePointer = null;
    pinch = pinchGeometry();
...
}, true); // capture: settle pinch state before the drag handler reads it
cnv.addEventListener('pointermove', e => {
  ...
  if (pinch.dist > 0 && now.dist > 0) {
    ...
    const world = { x: view.x + (now.mid.x - r.left) * view.w / r.width,
                    y: view.y + (now.mid.y - r.top) * view.h / r.height };
    zoomAt(now.dist / pinch.dist, world);
    // ...then translate by however far the midpoint itself travelled, which is
    // the pan half of the gesture.
    view.x -= (now.mid.x - pinch.mid.x) * view.w / r.width;
    view.y -= (now.mid.y - pinch.mid.y) * view.h / r.height;
  }
  pinch = now;
  renderDrag();
}, { passive: false });
function endPointer(e) {
  ...
  // Dropping to one finger must not resume the abandoned drag, and must not
  // start a pinch again from a stale distance.
...
// Fullscreen, with a fallback for the platform that needs it most.
//
// Safari on iPhone does not implement the Fullscreen API on ordinary elements
// at all: requestFullscreen is simply absent (only a video element can go
// fullscreen there). The button therefore did nothing, silently, on exactly the
// device where a full-viewport canvas is worth the most. iPadOS and every
// desktop browser do support it.
//
// So: use the real thing when it exists, and otherwise fall back to a
// fixed-position layer that fills the viewport. The fallback is not a lesser
// imitation for the user, it just cannot escape the browser chrome.
function pseudoFullscreen(on) {
  const wrap = document.querySelector('.cnvwrap');
  if (!wrap) return;
  wrap.classList.toggle('pseudofs', on);
  document.body.classList.toggle('pseudofs-host', on); // stops the page behind it scrolling
  syncCanvasHeight(); onCanvasResize();
}
function inPseudoFullscreen() {
  const wrap = document.querySelector('.cnvwrap');
  return !!wrap && wrap.classList.contains('pseudofs');
}
function toggleFullscreen() {
  const wrap = document.querySelector('.cnvwrap');
  if (!wrap) return;
  if (inPseudoFullscreen()) { pseudoFullscreen(false); return; }
  ...
  if (!req) { pseudoFullscreen(true); return; } // iPhone Safari lands here
  // catch: rejects when the embedding context denies fullscreen (an iframe
  // without allowfullscreen, or a browser that exposes the method but refuses
  // the request). Falling back beats doing nothing at all.
  try {
    const r = req.call(wrap);
    if (r && typeof r.catch === 'function') r.catch(() => pseudoFullscreen(true));
  } catch (err) { pseudoFullscreen(true); }
}
...
const CANVAS_HELP = [
  ...
  ['Zoom', 'Scroll, pinch with two fingers, or use the +/- buttons at the bottom right.'],
  ['Fullscreen', 'The last button in the zoom cluster. Press it again or Esc to leave.'],
