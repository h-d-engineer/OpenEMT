...
cnv.addEventListener('pointerdown', e => {
  ...
  if (t.dataset.term) {
    ...
    if (!S.wireFrom) S.wireFrom = parts;
      ...
      if (!(S.wireFrom[0] === parts[0] && S.wireFrom[1] === parts[1])) { pushHistory(); S.wires.push({ a: S.wireFrom, b: parts }); }
      S.wireFrom = null;
    }
    render(); return;
  }
...
function drawAllPlots() {
  ...
  S.plots.forEach(pl => drawOnePlot(pl, dark));
}
...
function tickLabels(r) {
  ...
  let dec = Math.max(0, Math.min(9, 1 - Math.floor(Math.log10(step))));
  let out = vals.map(v => v.toFixed(dec));
  while (dec > 0 && out.every(s => s.endsWith('0'))) { dec--; out = vals.map(v => v.toFixed(dec)); }
  // Fixed notation ran out of resolution (a nano-scale axis): every tick would
  // print the same string, so switch the whole axis to exponential.
  if (out[0] === out[4]) return vals.map(v => v.toExponential(2));
  return out.map((s, i) => (s.length > 11 ? vals[i].toExponential(2) : s));
}
