...
| [`ieee39bus.json`](ieee39bus.json) | **3-ph only** | The standard IEEE 39-bus New England system (10 generators, 12 transformers, 34 lines, 21 loads — 147 blocks total). Built from verified primary sources (MATPOWER `case39.m` for topology/dispatch, ANDES's GENROU sheet for generator H/Ra/Xd'), not reconstructed from memory. Loads are modeled as constant-impedance (`rlc` with R only, sized at nominal voltage; originally the standalone R block, since removed) rather than constant-power (`pq`) — at this system's per-phase power levels, `pq`'s cold-start behavior drove several buses to multi-million-volt divergence within the first cycle (see the 2026-07-15 DECISIONS.md entry); reactive load is therefore not represented, only real power. As the file ships (`Init from PF` on), all 39 bus voltages settle between **198.7 kV and 211.6 kV** phase, i.e. 0.997 to 1.062 pu against the 199.2 kV nominal, and all 10 generator frequencies within 0.15 Hz of 60 Hz. Eight buses sit just above 1.05 pu, and that is a direct consequence of the modeling choice above rather than a solver artifact: with the reactive half of the load dropped, the 34 lines contribute roughly 23 uF of charging with nothing to absorb it, and the ten machines all hold 1.000 pu, so the lightly loaded buses float up. Treat the profile as internally consistent but not as the canonical solved case; a study that cares about the voltage profile should add reactive load first. (An earlier version of this page quoted 164.5 to 199 kV. That was a **cold-start** run, where the machines swing in from rest, and 164.5 kV is a swing minimum rather than a settled value.) Second large-scale stress test after `ieee9bus.json`; largest model in this project to date. |
...
| [`single_phase_gfm_lateral.json`](single_phase_gfm_lateral.json) | **3-ph only** | A single-phase grid-following **GFM inverter** on a **phase-B** lateral (think rooftop solar or a battery inverter on one leg of a 277 V service), dispatching a 4 kW setpoint next to a balanced 3-phase service load. The case runs 1000 ms because that is what the grid-following PI actually needs: at 200 ms it is still swinging and delivering about 4.11 kW, and only by ~1 s does it settle onto its setpoint (4.00 kW, unchanged if you run it to 2 s). The inverter delivers that 4 kW on phase B alone, which lifts phase B's service-load current above phase A (15.141 vs 15.127 A settled): genuine single-phase unbalance from distributed generation, which no earlier block could produce. Note how small that unbalance is, and read it only from a settled run over a whole number of cycles: measured before the PI converges, the per-phase ordering is dominated by the swing rather than by the inverter, and it will flip depending on where you put the window. Probe #6 is the 277 V service bus, #14 the lateral. The inverter's DC port is left unwired here (idealized source); wire it to a battery-held bus for a real energy-limited study. Its AC current limiter is 3-phase only, so leave I ac max at 0. |
...
- `ieee_harmonics_14bus.json` **now also runs in EMT** (fixed 2026-08-14; it
  used to fail, and both reasons are worth knowing because they are the two
  standard obstacles to running any imported power-flow case as a transient).

  First it stopped with a singular matrix naming BUS 8 and transformer #67. Two
  of its units are `Yd11`, wye primary into a **delta secondary**, and a delta
  has no neutral: nothing else on those two buses reached ground, so the winding
  set genuinely had no voltage reference. Each delta secondary is now grounded
  through a 1 MOhm resistance, which is what high-resistance grounding is and
  what a real delta-fed 13.8 kV or 35.4 kV system actually has. A resistance
  rather than a capacitance on purpose: a capacitance would add a resonance to a
  case whose entire subject is harmonic resonance. It shifts the power flow by
  1.2e-6 pu, which is to say not at all.

  Then it diverged immediately, reaching 400 pu within a millisecond. Its 11
  loads were **constant-power**, which is correct for a load flow and wrong for
  a transient: a constant-power load behind an impedance is a positive-feedback
  element, and the solver's own error message names it. They are now `zip` loads
  with pure constant-impedance weights (`az`/`bz` = 1), the conventional load
  model for both stability and harmonic studies. The published power flow is
  preserved exactly: the solve still converges in 7 iterations and still
  reproduces every one of the RAW's 16 bus magnitudes and angles.

  The two `Yd11` units are the point of the case and were left alone. BUS 301
  and BUS 302 carry identical loads behind a `Yy0` and a `Yd11` respectively, so
  the pair exists precisely to compare how the two vector groups pass harmonics;
  changing either connection to make the matrix solvable would have deleted the
  experiment.
